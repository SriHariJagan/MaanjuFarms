...
const UserOrders = () => {
  ...
  const currentData = useMemo(() => {
    return activeTab === "orders" ? orders : bookings;
  ...
  return (
    <motion.div className={styles.userOrders} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
      <motion.div className={styles.header} variants={fadeUp} initial="initial" animate="animate">
        <div>
          <h2>{activeTab === "orders" ? "My Product Orders" : "My Villa Bookings"}</h2>
          <p>Track your orders and booking status</p>
        </div>
        <div className={styles.headerRight}>
          <div className={styles.switchTabs}>
            <button className={activeTab === "orders" ? styles.activeTab : ""} onClick={() => setActiveTab("orders")}>
              <Package size={18} />Product Orders
            </button>
            <button className={activeTab === "bookings" ? styles.activeTab : ""} onClick={() => setActiveTab("bookings")}>
              <Building2 size={18} />Villa Bookings
            </button>
          </div>
          <div className={styles.totalOrders}>{currentData.length} Records</div>
        </div>
      </motion.div>

      {activeTab === "orders" && (
        <motion.div className={styles.ordersWrapper} variants={stagger} initial="initial" animate="animate">
          {orders.map((order) => {
              ...
              <motion.div className={styles.orderCard} key={order._id} variants={fadeUp} layout>
                <div className={styles.orderSummary} onClick={() => toggleOrder(order._id)}>
                  ...
                  <div className={styles.summaryCenter}>
                    <div><label>Total</label><h4>₹{order.totalAmount}</h4></div>
                    <div><label>Products</label><h4>{order.products.length}</h4></div>
                  </div>
                ...
                <AnimatePresence>
                  {isOpen && (
                    <motion.div className={styles.orderDetails} variants={slideDown} initial="initial" animate="animate" exit="exit">
                      <div className={styles.section}>
                        <div className={styles.sectionTitle}>Ordered Products</div>
                        <div className={styles.productsWrapper}>
                          {order.products.map((item) => (
                            <div className={styles.productItem} key={item._id}>
                              <div className={styles.productImageWrapper}><img src={getImageUrl(item.product?.image)} alt="" /></div>
                      ...
                      <div className={styles.paymentSection}>
                        ...
                        <div>
                          ...
                          <div className={styles.statusInfo}>
                            {order.status === "delivered" ? <BadgeCheck size={18} /> : <Truck size={18} />}
                            <p>{order.status}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      {(order.trackingId || order.courierName) && (
                        <div className={styles.section}>
                          ...
                          <div className={styles.shippingBox}>
                            {order.trackingId && (
                              <div className={styles.shippingItem}><label>Tracking ID</label><p>{order.trackingId}</p></div>
                            )}
                            {order.courierName && (
                              <div className={styles.shippingItem}><label>Courier</label><p>{order.courierName}</p></div>
                            )}
                          </div>
                        </div>
                      )}
            ...
            return (
              <motion.div className={styles.orderCard} key={booking._id} variants={fadeUp} layout>
                <div className={styles.orderSummary} onClick={() => toggleOrder(booking._id)}>
                  <div className={styles.summaryLeft}>
                    <div className={styles.orderIcon}><Building2 size={22} /></div>
                    <div>
                  ...
                  <div className={styles.summaryCenter}>
                    <div><label>Total</label><h4>₹{booking.totalAmount}</h4></div>
                    <div><label>Guests</label><h4>{booking.guests}</h4></div>
                  </div>
                  <div className={styles.summaryRight}>
                  ...
                  {isOpen && (
                    <motion.div className={styles.orderDetails} variants={slideDown} initial="initial" animate="animate" exit="exit">
                      <div className={styles.roomCard}>
                        <img src={getImageUrl(booking.room?.images?.[0])} alt="" />
                      ...
                      <div className={styles.bookingGrid}>
                        <div className={styles.bookingInfo}>
                          ...
                          <div><label>Check In</label><p>{new Date(booking.checkIn).toLocaleDateString()}</p></div>
                        </div>
                        <div className={styles.bookingInfo}>
                          <CalendarDays size={18} />
                          <div><label>Check Out</label><p>{new Date(booking.checkOut).toLocaleDateString()}</p></div>
                        </div>
                        <div className={styles.bookingInfo}>
                          <Users size={18} />
                          <div><label>Guests</label><p>{booking.guests} Guests</p></div>
                        </div>
                        <div className={styles.bookingInfo}>
                          <CreditCard size={18} />
                          <div><label>Payment</label><p>{booking.paymentStatus}</p></div>
                        </div>
                      </div>
