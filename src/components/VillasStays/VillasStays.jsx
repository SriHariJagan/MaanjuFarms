...
const VillasStays = () => {
  const { villas, bookings, loading, error, fetchVillas, bookVilla, updateVilla } = useVillas();
  ...
  const handleCheckAvailability = () => {
    if (!checkIn || !checkOut) {
      ...
      toast.warn("Please select check-in and check-out dates", { toastId: "select-dates" });
      return;
    }
    ...
    if (inDate < today.setHours(0, 0, 0, 0)) {
      ...
      toast.error("Check-in cannot be in the past", { toastId: "past-checkin" });
      return;
    }

    if (outDate <= inDate) {
      setAvailability("Check-out must be after check-in");
      toast.error("Check-out must be after check-in", { toastId: "invalid-range" });
      return;
    }

    if (selectedVilla.status === "maintenance") {
      setAvailability("Villa unavailable (Maintenance)");
      toast.info("Villa is under maintenance", { toastId: `maintenance-${selectedVilla._id}` });
      return;
    }
    ...
    if (overlap) {
      ...
      toast.info("Villa is already booked for these dates", { toastId: `booked-${selectedVilla._id}` });
    } else {
      setAvailability("Available");
      toast.success("Villa is available for selected dates", { toastId: `available-${selectedVilla._id}` });
    }
  };
  ...
  const handleBookNow = async () => {
    ...
    if (availability !== "Available") {
      toast.error("Villa is not available for the selected dates", { toastId: `not-available-${selectedVilla._id}` });
      return;
    }
      ...
      <div className={styles.container}>
        <div className={styles.hero}>
          ...
          <p className={styles.subtext}>
            Immerse yourself in the tranquility of Maanjoo Farms with our exclusive stays. Each villa is designed to offer comfort, luxury, and a connection to nature.
          </p>
        </div>
          ...
          {villas.map((villa) => (
            <div key={villa._id} className={styles.card}>
              ...
              <div className={styles.content}>
                ...
                <p>
                  Status:{" "}
                  <span className={getVillaStatus(villa) === "Available" ? styles.available : styles.unavailable}>
                ...
                {isAdmin && (
                  <select
                    ...
                    value={villa.status}
                    onChange={(e) => handleStatusChange(villa._id, e.target.value)}
                  >
                 ...
                 <button className={styles.btn} onClick={() => handleOpenModal(villa)}>
                    Check Availability
                  </button>
              </div>
            </div>
          ...
          <div className={styles.modalOverlay}>
            <div className={styles.modal}>
              ...
              <div className={styles.modalContent}>
                ...
                {availability && (
                  <p className={availability === "Available" ? styles.available : styles.unavailable}>
                    {availability}
                  </p>
                )}
                ...
                {bookingStatus && (
                  <p className={bookingStatus.includes("success") ? styles.available : styles.unavailable}>
                    {bookingStatus}
                  </p>
                )}
                ...
                <div className={styles.upcomingBookings}>
                  ...
                  {bookings
                    .filter((b) => b.room?._id === selectedVilla._id && new Date(b.checkOut) >= new Date())
                  ...
                  {bookings.filter((b) => b.room?._id === selectedVilla._id && new Date(b.checkOut) >= new Date()).length === 0 && (
                    <p>No upcoming bookings for this villa.</p>
                  )}
                </div>
              </div>
            </div>
